 ...
 */
// ADJUSTABLE CONFIGS
const ASCEND_LIMIT = 50 //After this level of ascension, gang member wont ascend anymore.
const ASCEND_MULT_LIMIT = 25 //Unless they can ascend this multiplier more.
const ASCEND_POWER_THRESHOLD = 15 //Ascend multiplier the gang member should be at before contributing to territory warfare
const RESPECT_LIMIT = 5000000
const STR_MULTIPLIER_THRESHOLD = 3
...
// VARS - DO NOT TOUCH
const TRAINING_TASK = "Train Combat"
...
const FOCUS = { "respect": "Respect", "wanted": "Wanted Level", "money": "Money Gain" }
...
/** @param {NS} ns
 * recruit gang members if possible.
*/
function recruit(ns) {
...
function ascend(ns, mem) {
	const info = ns.gang.getMemberInformation(mem)
	const str_mult = ns.formulas.gang.ascensionMultiplier(
	        ns.formulas.gang.ascensionPointsGain(info.str_exp) +
	        info.str_asc_points
	    ) - info.str_asc_mult + 1
	// after ascend limit, stop ascending member.
	// HOWEVER, if multiplier hits the ascend multiplier limit, ascend again.
	// Allows for stronger Gs while only gradually impacting respect long term.
	if (info.str_asc_mult > ASCEND_LIMIT && str_mult <= ASCEND_MULT_LIMIT) {
		return
	}
  // If the strength ascension multiplier is above 5, ascend the member and set their task back to training.
	if (str_mult > STR_MULTIPLIER_THRESHOLD) {
 ...
 * in 1200 ticks, set that task as their current focus.
*/
...
/** @param {NS} ns
 * Determine which task the member should take.
 * Whether the focus is respect or money, find the task with the highest gain and return it.
*/
...
/** @param {NS} ns
 * Set and determine the member's primary task, depending on their stats and
*/
function set_focus(ns, info, mem) {
	...
	if (ns.gang.getGangInformation().moneyGainRate > 5000000)
...
function buy_equipment(ns, mem) {
	for (var equip of ns.gang.getEquipmentNames()) {
		if (EQUIP_TYPES.includes(ns.gang.getEquipmentType(equip)) && ns.gang.getEquipmentCost(equip) < ns.getPlayer().money / 100) {
