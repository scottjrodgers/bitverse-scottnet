...
function walk(ns, name, servers, previous = undefined) {
  ...
  // ns.tprintf("walking: %s, previous: %s, map-size: %d", name, previous, servers.size);
...
/** @param {NS} ns */
export function getHacknetFreeRam(servers) {
  let total = 0;
  for (let [name, server] of servers) {
    if (name.startsWith("hacknet")) {
      total += server.freeRam();
...
export function get_network(ns) {
  ...
  // ns.tprintf("Resulting Servers:");
  // ns.tprintf("%s, size: %d", JSON.stringify(servers.keys()), servers.size);
  return servers;
}
