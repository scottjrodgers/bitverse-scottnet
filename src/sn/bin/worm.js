...
function display(ns, host, limit){
  ...
  if (reqdHack <= limit){
    ...
    // } else {
      ...
      ns.tprintf("%s: Hack: %d, Ports: %d, Max Cash: %s, Curr. Cash: %s, Min Security: %d, Curr. Security: %d, Money/Security: %s, HackXP: %s, HackXP/HackTime: %s, HackXP/MinSecurity: %s",
        host, reqdHack, numPorts, maxMoney, currMoney, minSecurity, currSecurity, moneyRatio, ns.formatNumber(h_exp), 
        ns.formatNumber(1000 * h_exp / h_time), ns.formatNumber(1000 * h_exp/minSecurity));
    // }
  }
}
