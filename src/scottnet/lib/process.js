...
import {calculateServerRatio} from "./core";
...
export class ProcessInfo {
    /** @param {NS} ns */
    constructor(ns, filename, args, threads) {
        this.ns = ns;
...
export class BotBatch {
    constructor(botType, numJobs, period, threadsPerJob) {
...
/** @param {NS} ns */
export function collect_processes(ns, servers, by_server = false) {
    ...
    for (let host of servers.keys()) {
        ...
        for (let p of processes) {
            ...
            let process = new ProcessInfo(ns, filename, p.args, p.threads);
...
/** @param {NS} ns */
export function deploy_job(ns, bot, args = [], n = 1, threads = 1, delay = 0, gap = 0, verbose = false) {
